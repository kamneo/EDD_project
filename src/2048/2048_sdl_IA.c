...
int main(int argc, char *argv[])
{
	...
	dir direction;				
	strategy strat;			// instance de la stratégie// contient la direction à jouer pour chaque tour
	grid g;
...
/*	Affichage des tiles */
	for(int x=0;x<GRID_SIDE;x++)							// Boucle de parcourt de la grille
	{
		for (int y=0;y<GRID_SIDE;y++)
		{
			...
			SDL_FillRect(tile_Sdl,NULL,ColorsTab[valTile%(NB_COLOR-1)]);					// on colorie la surface de la couleur correspondante à sa valeur	
